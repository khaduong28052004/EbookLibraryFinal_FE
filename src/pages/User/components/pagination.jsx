...
const Pagination = ({
  ...
  return (
    <div className="py-6 flex border-stroke dark:border-strokedark px-4 md:px-6 xl:px-7.5">
      {/* Mobile view */}
      ...
      <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
        {/* <div>
          <p className="text-sm text-gray-700 dark:text-white">
            Showing
            <span className="font-medium"> {pageNumber * 7 + 1} </span>
            to
            <span className="font-medium"> {Math.min((pageNumber + 1) * 7, totalElements)} </span>
            of
            ...
            {[...Array(totalPages)].map((_, index) => (
              <button
                ...
                className={`relative z-10 inline-flex items-center px-4 py-2 text-sm font-semibold ${
                  index === pageNumber ? 'bg-[#003EA1] text-white' : 'text-gray-900'
