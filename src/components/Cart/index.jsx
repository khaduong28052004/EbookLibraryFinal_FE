...
export default function Cart({ className, type, data }) {
    ...
    if (!isRequesting) {
      ...
      data?.forEach(seller => {
        seller?.cart?.forEach(cart => {
          total += (cart?.product?.price * ((100 - (cart?.product?.sale)))) * cart.quantity
        })
      });
      setSubtotal(total);
    }
        ...
        <div className="w-full h-full">
          <div className="product-items h-[310px] overflow-y-scroll">
            <ul>
              {data?.map((seller, sellerIndex) => (
                seller?.cart?.map((cartItem, cartIndex) => (
                  <li key={`${sellerIndex}-${cartIndex}`} className="w-full h-full flex">
                    <div className="flex space-x-[6px] justify-center items-center px-4 my-[20px]">
                      <div className="w-[65px] h-full">
                        <img
                          src={cartItem?.product?.imageProducts[0]?.name} // Ideally use cartItem.image if available
                          alt="Product"
                      ...
                      <div className="flex-1 h-full flex flex-col justify-center">
                        <p className="title mb-2 text-[13px] font-600 text-qblack leading-4 line-clamp-2 hover:text-blue-600">
                          {cartItem?.product?.name}<sup className="text-red-600 text-[10px]">x{cartItem.quantity}</sup>
                        </p>

                        <p className="price">
                          <span className="offer-price text-qred font-600 text-[15px] ml-2">
                            {Intl.NumberFormat().format(cartItem.product.price * ((100 - (cartItem.product.sale)) / 100))}<sup>đ</sup>
                          </span>
                        </p>
                      </div>
                    </div>
          ...
          <div className="product-actions px-4 mb-[30px]">
            <div className="total-equation flex justify-between items-center mb-[28px]">
              ...
              <span className="text-[15px] font-500 text-qred ">{Intl.NumberFormat().format(subtotal / 100)}<sup>đ</sup></span>
            </div>
            <div className="product-action-btn">
              ...
              <a href="/cart">
                <div className="w-full h-[50px]">
                  <div className={type === 3 ? "blue-btn" : "blue-btn"}>
          ...
          <div className="flex justify-center py-[15px]">
            <p className="text-[13px] font-500 text-qgray">
              Nhận lại trong vòng <span className="text-qblack">30 ngày</span>
            </p>
          </div>
        </div>
      </div>
